...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        loss_ce = F.cross_entropy(src_logits.transpose(1, 2).contiguous(), target_classes, self.empty_weight)
    ...
    def loss_masks(self, outputs, targets, indices, num_boxes):
        ...
        target_masks = target_masks.view(src_masks.shape).contiguous()
        losses = {
...
def accuracy(output, target, topk=(1,)):
    ...
    pred = pred.t()
    correct = pred.eq(target.view(1, -1).contiguous().expand_as(pred))
